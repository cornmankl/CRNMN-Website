      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {crnmnLocations.map((location) => {
          ...
          return (
            <Card
              key={location.id}
              className={`bg-[var(--neutral-800)] border-[var(--neutral-700)] hover:border-[var(--neon-green)] transition-colors cursor-pointer ${selectedLocation?.id === location.id ? 'border-[var(--neon-green)]' : ''
                }`}
              onClick={() => {
                setSelectedLocation(location);
                onLocationSelect?.(location);
